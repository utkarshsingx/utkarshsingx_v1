...
function calcHeight(
  ...
  const rScore = Math.pow(rNorm, 0.5);
...
export function generateCityLayout(devs: DeveloperRecord[]): {
  ...
  function placeBlockContent(
    ...
    for (let i = 0; i < blockDevs.length; i++) {
      ...
      const devRecord = dev as unknown as Record<string, unknown>;
      ...
      buildings.push({
        ...
        achievements: (devRecord.achievements as string[]) ?? [],
        kudos_count: (devRecord.kudos_count as number) ?? 0,
        visit_count: (devRecord.visit_count as number) ?? 0,
        loadout: (devRecord.loadout as CityBuilding['loadout']) ?? null,
        app_streak: (devRecord.app_streak as number) ?? 0,
        raid_xp: (devRecord.raid_xp as number) ?? 0,
        current_week_contributions: (devRecord.current_week_contributions as number) ?? 0,
        current_week_kudos_given: (devRecord.current_week_kudos_given as number) ?? 0,
        current_week_kudos_received: (devRecord.current_week_kudos_received as number) ?? 0,
        active_raid_tag: (devRecord.active_raid_tag as CityBuilding['active_raid_tag']) ?? null,
        active_drop: null,
        rabbit_completed: (devRecord.rabbit_completed as boolean) ?? false,
        xp_total: (devRecord.xp_total as number) ?? 0,
        xp_level: (devRecord.xp_level as number) ?? 1,
        district: did,
        district_chosen: (devRecord.district_chosen as boolean) ?? false,
